...
> ⚠️ **請勿刪除此 CNAME 記錄**
>
> 此 CNAME 是憑證自動續約的委派依據。**一旦刪除，下次自動續約的 DNS-01 驗證會失敗、憑證無法更新**，可能導致服務憑證過期。除非確定要停用自動續約，否則請長期保留。
>
> 注意:此提醒**只針對 `_acme-challenge.*` 這筆 CNAME**。域名的 **A record（指向伺服器 IP）是服務連線用,與 ACME 驗證無關** —— DNS-01 只看 `_acme-challenge` 這筆 CNAME / TXT,不看 A record,兩者請勿混淆。
